...
void CPUSolver::transferAllInterfaceFluxes() {
  ...
  while (true) {
    ...
    _timer->stopTimer();
    _timer->recordSplit("Transfer needs communication time");
#endif
...
void CPUSolver::printLoadBalancingReport() {
  ...
  // Use rank 0 positioning at (0,0,0) to get index in _boundary_tracks
  int id_domain = 0;
  int domain = _geometry->getNeighborDomain(0, 0, 1);
  if (domain != -1) {
    tracks_z = _boundary_tracks.at(id_domain).size();
    id_domain++;
  }
  domain = _geometry->getNeighborDomain(0, 1, 0);
  if (domain != -1) {
    tracks_y = _boundary_tracks.at(id_domain).size();
    id_domain++;
  }
  domain = _geometry->getNeighborDomain(1, 0, 0);
  if (domain != -1)
    tracks_x = _boundary_tracks.at(id_domain).size();
